...
|| *exit();*         As the name implies, this function will allow the user to exit their current state in the program. || Our rShell class now uses and tests for *precedence* when parsing through input from the user by using the test executable. Our rShell also runs literal and symbolic tests via the test executable.
